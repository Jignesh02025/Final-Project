...
const CartItems = () => {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${Backend_Url}create-checkout-session`, { Amount: Amount, user_id: cUser });
    ...
    return (
        <div className='cartitem'>
            ...
            {Array.isArray(cartdata) && cartdata.length > 0 ?
                (cartdata.map((e, inxes) => {
                    return (
                        <div key={e._id}>
                            <div className="cartitem-format cartitem-format-main">
                                <img src={e.image} className='cartitem-product-icon' alt="" onClick={() => {
                                ...
                                <p>${e.total}</p>
                                <img className='cartitem-remove-icon' src={remove_icon} onClick={() => { removefromcart(e) }} alt="" />
                            </div>
                            <hr />
                        </div>
                    )
